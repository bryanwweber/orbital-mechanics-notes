...
This gives velocities of $v_1 =$ {eval}`v_1:.3f` km/s and $v_3 =$ {eval}`v_3:.3f` km/s.
...
This gives $e_t =$ {eval}`e_t:.4f`, $h_t =$ {eval}`h_t:.3f` km<sup>2</sup>/s, and $a_t =$ {eval}`a_t:.4g` km. Now, the velocity at the perigee and apogee of the transfer orbit are determined from the specific orbital angular momentum.
...
The velocity at the apogee of the transfer orbit is $v_{a,t} =$ {eval}`v_a_t:.3f` km/s, but the orbital velocity at the Moon's orbit is over 1 km/s, a factor of nearly five. Now, let's calculate $\Delta v$:

```{code-cell} ipython3
delta_v_t = abs(v_p_t - v_3) + abs(v_1 - v_a_t)
```

The $\Delta v$ for the two-impulse Hohmann transfer is $\Delta v =$ {eval}`delta_v_t:.3f` km/s.
...
These velocities are $v_{p,t1} =$ {eval}`v_p_t1:.3f` km/s, $v_{a,t1} =$ {eval}`v_a_t1:.3f` km/s, $v_{p,t2} =$ {eval}`v_p_t2:.3f` km/s, and $v_{a,t2} =$ {eval}`v_a_t2:.3f` km/s.
...
For the bi-elliptic transfer, $\Delta v =$ {eval}`delta_v_b:.3f` km/s, about {eval}`delta_v_diff:.0f` m/s, or {eval}`delta_v_percent:.2f`% less than the two-impulse Hohmann transfer.
...
The difference is {eval}`delta_m_diff:.1f` kg per 1000 kg of spacecraft mass.
...
The transfer time for the two-impulse Hohmann transfer is $t_h =$ {glue:text}`t_h/86_400:.3f` days, and for the bi-elliptic transfer it is $t_{be} =$ {glue:text}`t_be/86_400:.3f` days. The transfer time is almost {glue:text}`t_be/t_h:.0f`x longer for the bielliptic transfer!
